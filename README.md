...
## 궁금한 점
- api에서 max count가 넘었음에도 아이템이 계속 내려오는 건 오류일까? 혹은 테스트를 위한 장치일까?
- api키 값이 정규화 되어있지 않고 몇개는 snake case를 쓰는데 어떤 기준일까?
- QR인식과 웹뷰 이동은 라이브러리 의존 성향이 짙은데 이 부분은 어떤 역량을 평가하기위해 과제에 추가됐을까?
